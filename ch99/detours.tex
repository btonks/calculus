...
every member of Q, which amounts to an assumption that the function only crosses
through $y_3$ once. Using the completeness axiom, there exists some real $x$ which is greater than or equal
to every element of P and less than or equal to every element of Q.
Now $x$ must belong either to P or to Q, or both. Suppose it belongs to P.
Then the following
...
so that if $\der x$ is infinitesimal and $x'=x+\der x$, we have $y(x)<y_3$, but $y(x+\der x)>y_3$.
