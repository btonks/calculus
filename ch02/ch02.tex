...
powers. This is a special case of the geometric series\index{geometric series}
\begin{equation*}
       \frac{1}{1-t} = 1+t+t^2+\ldots \qquad ,
\end{equation*}
which can be derived by doing synthetic division\index{synthetic division} (the equivalent of long
...
As we'll see in section \ref{sec:safe-infinitesimals}, infinitesimals obey all the same elementary laws of algebra as the real numbers,
...
\begin{align*}
  \frac{\der x}{\der t} &= \frac{\left(1+\der t+{\der t}^2+\ldots\right)-1}{1+\der t-1} \\
                        &= 1+\ldots \qquad ,
\end{align*}
where as usual we discard infinitesimal terms at the end.
\end{eg}

Example \ref{eg:geometric-series} required some
ingenuity. By the end of this chapter you'll learn general techniques for
cranking out any derivative cookbook-style.
%
...
\frac{\der x}{\der t}   &= \frac{\cos t \: \der t}{\der t} \\
                        &= \cos t \qquad .
\end{align*}
This is all there is to the computation of the derivative. 
But were the approximations good enough?
Being a little less cavalier, I should demonstrate
...
infinitesimal size of $\der t$. A quick and dirty way to check whether this is likely to be true is to
use Inf to calculate $\sin(t+\der t)$ at some specific value of $t$. For example, at $t=1$ we have
this result:\footnote{If you're wondering how Inf calculates this kind of thing, the answer is that it uses something called
...
On page \pageref{detour:sin-rigor} I've given a general proof that works without having to pick a specific
value of $t$ to check.\index{derivative!of the sine}\index{sine!derivative of}
...
1. Numerical calculation also shows that, for example, $e^{0.001}=1.00100050016671$ is very close
